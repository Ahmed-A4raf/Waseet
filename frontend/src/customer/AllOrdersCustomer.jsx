...
const AllOrdersCustomer = () => {
  ...
  if (loading) return <div className="pt-24 text-center text-lg dark:text-zinc-100">Loading...</div>;
  if (error) return <div className="pt-24 text-center text-red-500 dark:text-red-400">{error}</div>;
        ...
        <table className="min-w-full table-auto text-sm text-gray-700 dark:text-zinc-200">
          ...
          <tbody>
            {orders.map((order, idx) => (
              ...
              >
                <td className="border px-6 py-4 dark:border-zinc-700">{order.id}</td>
                <td className="border px-6 py-4 dark:border-zinc-700">{order.buyerEmail}</td>
                <td className="border px-6 py-4 dark:border-zinc-700">
                  {new Date(order.orderDate).toLocaleDateString()}
                </td>
                <td className="border px-6 py-4 dark:border-zinc-700">{order.status}</td>
                <td className="border px-6 py-4 dark:border-zinc-700 font-semibold">{order.total} $</td>
                <td className="border px-6 py-4 dark:border-zinc-700">
                ...
                <td className="border px-6 py-4 dark:border-zinc-700">
                  {order.orderItems.map((item, idx) => (
                    <div key={idx} className="mb-2">
                      <p className="font-medium">{item.productItemOrder.productName}</p>
