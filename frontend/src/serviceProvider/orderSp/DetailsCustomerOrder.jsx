  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        ...
        if (matchedOrder) {
          matchedOrder.orderItems = matchedOrder.orderItems.filter(
            (item) =>
              item.productItemOrder.serviceProviderEmail === providerEmail
          );
        }
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 lg:px-8">
        {loading ? (
          ...
          className="bg-white dark:bg-zinc-800 p-6 rounded-lg shadow space-y-6">
            <h2 className="text-2xl font-bold text-gray-800 dark:text-zinc-50">Order #{orderId}</h2>

            <div className="text-gray-700 dark:text-zinc-200">
              <h3 className="font-semibold text-lg mb-1">Customer Info</h3>
            ...
            <div>
              ...
              <ul className="space-y-4">
                {order.orderItems?.map((item) => (
                  <li key={item.id} className="flex items-center gap-4 border-b border-gray-200 dark:border-zinc-700 pb-4">
                    <img
...
export default DetailsCustomerOrder;
